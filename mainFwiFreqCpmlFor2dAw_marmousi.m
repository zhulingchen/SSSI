% MAINFWIFREQCPMLFOR2DAW simulates the full wave inversion (FWI)
%
...
clear;
clc;
%% Full Wave Inversion Example - Marmousi Model
% in frequency domain
...
filenameDataTrueFreq = './modelData/marmousiModelData/marmousi_dataTrueFreq.mat';
filenameDataDeltaFreq = './modelData/marmousiModelData/marmousi_dataDeltaFreq0.mat';
...
while(norm(modelNew - modelOld, 'fro') / norm(modelOld, 'fro') > DELTA && iter <= MAXITER)
    ...
    parfor iw = activeW
        ...
        tic;
        [A, ~] = freqSolverCpml(vmOld, zeros(size(V)), w(iw), nDiffOrder, nBoundary, dz, dx);
    ...
    filenameHessianDiag = sprintf('./modelData/marmousiModelData/marmousi_hessianDiag%d.mat', iter);
    save(filenameHessianDiag, 'hessianDiag', '-v7.3');
    
    % save the migrated image
    filenameMig = sprintf('./modelData/marmousiModelData/marmousi_mig%d.mat', iter);
    save(filenameMig, 'mig', '-v7.3');
    
    lambda = 5e-3 * max(hessianDiag);
    ...
    filenameVmNew = sprintf('./modelData/marmousiModelData/marmousi_vmNew%d.mat', iter);
    ...
    %     colormap(seismic);
    %     % save current updated velocity model
    %     % filenameVmNew = sprintf('./modelData/marmousiModelData/vmNew%d.mat', iter);
    %     % save(filenameVmNew, 'vmNew', 'modelNew', '-v7.3');
    %
    %     dataDeltaFreq = zeros(nRecs, nShots, nfft);
    %% debug end
    ...
    filenameDataDeltaFreq = sprintf('./modelData/marmousiModelData/marmousi_dataDeltaFreq%d.mat', iter);
    save(filenameDataDeltaFreq, 'dataDeltaFreq', '-v7.3');
