% MAINBORNAPPROXCURVELETFWIFREQCPMLFOR2DAW simulates the full waveform
...
% model perturbation is assumed sparse under the Curvelet transform.
%
%
% System background
% ====================================================================================================
...
% velocity model m
% delta_m(x) is assumed sparse under the Curvelet transform:
% delta_m(x) = Phi(c) where Phi is the synthesis Curvelet operator
%
% ====================================================================================================
%
%
...
while(norm(modelNew - modelOld, 'fro') / norm(modelOld, 'fro') > DELTA && iter <= MAXITER)
    ...
    if ~isunix
        curveletCoeff = fdct_wrapping(modelOld, is_real, 2, nbscales, nbangles_coarse);
    else
    ...
    %% updated model
    dcoeff = dcoeff_pqn_curvelet;
    dm = dm_pqn_curvelet;
    misfit = misfit_pqn_curvelet;
    ...
    filenameVmNew = [pathVelocityModel, sprintf('/vmNew%d_curvelet.mat', iter)];
