% MAINBORNAPPROXSOTFWIFREQCPMLFOR2DAW simulates the full waveform inversion
% (FWI) with 2-d acoustic wave in frequency domain based on the CPML
% absorbing boundary condition, the Born approximation and the sparse
% orthonormal transform (SOT).
%
...
% model perturbation is assumed sparse under the SOT.
%
%
% System background
% ====================================================================================================
...
% velocity model m
% delta_m(x) is assumed sparse under the SOT:
% delta_m(x) = D * c where D is the synthesis (inverse) SOT operator
%
% ====================================================================================================
%
%
...
% block size
blkSize = 20;
% initial dictionary (identity matrix)
D = eye(blkSize * blkSize, blkSize * blkSize);
A = zeros(blkSize * blkSize, blkSize * blkSize);
% number of training blocks
nBlocks = 2000;
% options
sotOpt = struct('verbosity', 0, 'lambda', 0.2, 'iterations', 100, 'tol', 1e-3);
...
while(norm(modelNew - modelOld, 'fro') / norm(modelOld, 'fro') > DELTA && iter <= MAXITER)
    ...
    % SOT
    sotFunc = @(x, mode) wrapper_sot(x, D, nz + nBoundary, nx + 2*nBoundary, mode);
    ...
    %% minimization using PQN toolbox in SOT domain
    func = @(dcoeff) lsBornApproxMisfitTransform(dcoeff, @(x)sotFunc(x, 1), @(x)sotFunc(x, 2), w(activeW), rw1dFreq(activeW), dataDeltaFreq, greenFreqForShotSet, greenFreqForRecSet);
    % tau = 1e-4;
    ...
    [dcoeff_pqn_sot, misfit_pqn_sot] = minConF_PQN_new(func, zeros(nLengthWithBoundary, 1), funProj, options);
    dm_pqn_sot = real(sotFunc(dcoeff_pqn_sot, 1));
    
    
    %% updated model
    dcoeff = dcoeff_pqn_sot;
    dm = dm_pqn_sot;
    misfit = misfit_pqn_sot;
    ...
    filenameVmNew = [pathVelocityModel, sprintf('/vmNew%d_sot.mat', iter)];
    save(filenameVmNew, 'vmNew', 'modelNew', 'dcoeff', 'dm', 'D', '-v7.3');
    ...
    % online dictionary learning (update D)
    [D, ~, A] = onlineSotTrain(reshape(dm, nz + nBoundary, nx + 2*nBoundary), D, iter, A, blkSize, nBlocks, sotOpt);
    sotFunc = @(x, mode) wrapper_sot(x, D, nz + nBoundary, nx + 2*nBoundary, mode);
