...
dt = 0.75*min(min(dz./velocityModel/sqrt(2)));
...
for ixs = 1:nShots %21:nx+20 % shot loop
    ...
    tic;
    [dataTrue, snapshotTrue] = fwdTimeSpongeFor2dAw(V, source, nBoundary, dz, dx, dt);
    [dataSmooth, snapshotSmooth] = fwdTimeSpongeFor2dAw(VS, source, nBoundary, dz, dx, dt);
    timeForward = toc;
...
for ixs = 1:nShots      %1:nx
    ...
    tic;
    [~, rtmsnapshot] = rvsTimeSpongeFor2dAw(V, dataDelta, nBoundary, dz, dx, dt);
    timeRT = toc;
