...
clear;
clc;
%% Full Wave Inversion Example - Fault Model
% in frequency domain
...
load('./modelData/faultModelData/velocityModelFaultSmooth_close.mat'); % velocityModelSmooth
...
filenameDataTrueFreq = './modelData/faultModelData/fault_dataTrueFreq.mat';
filenameDataDeltaFreq = './modelData/faultModelData/fault_dataDeltaFreq0.mat';
...
while(norm(modelNew - modelOld, 'fro') / norm(modelOld, 'fro') > DELTA && iter <= MAXITER)
    ...
    filenameHessianDiag = sprintf('./modelData/faultModelData/fault_hessianDiag%d.mat', iter);
    save(filenameHessianDiag, 'hessianDiag', '-v7.3');
    
    % save the migrated image
    filenameMig = sprintf('./modelData/faultModelData/fault_mig%d.mat', iter);
    ...
    filenameVmNew = sprintf('./modelData/faultModelData/fault_vmNew%d.mat', iter);
...
%     colormap(seismic);
%     % save current updated velocity model
%     % filenameVmNew = sprintf('./modelData/faultModelData/vmNew%d.mat', iter);
%     % save(filenameVmNew, 'vmNew', 'modelNew', '-v7.3');
%     
%     dataDeltaFreq = zeros(nRecs, nShots, nfft);
    %% debug end
    ...
    filenameDataDeltaFreq = sprintf('./modelData/faultModelData/fault_dataDeltaFreq%d.mat', iter);
    save(filenameDataDeltaFreq, 'dataDeltaFreq', '-v7.3');
