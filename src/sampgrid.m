...
%  BLOCKSIZE from the dim-dimensional matrix X, arranging the samples as the
...
%  BLOCKSIZE should either be a dim-element vector of the form [N1,N2,...Np],
...
dim = ndims(x);
if (dim==2 && any(size(x)==1) && length(blocksize)==1)
    dim = 1;
end

if (numel(blocksize)==1)
    blocksize = ones(1,dim)*blocksize;
end
...
for k = 1:nsamps
    ...
    if (k<nsamps)
        j = 1;
        while (ids(j) == n(j))  % time to start from a new column of blocks
            ids(j) = 1;
            block_ids{j} = varargin{j}(1) : varargin{j}(1)+blocksize(j)-1;
            j = j+1;
        end
