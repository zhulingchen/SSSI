function [snapshotVzp, snapshotVxp, snapshotVzs, snapshotVxs] = modified_fwdTimeSpmlFor2dEw(vp, vs, source, nDiffOrder, nBoundary, dz, dx, dt, izs, ixs)
%
...
% Modified by Lijun Zhu (gatechzhu@gmail.com) for large size simulation
...
nxs = length(ixs);
if (nzs > nz) || (nxs > nx)
    error('Too many sensor locations');
end

snapshotVzp = zeros(nzs, nxs, nt);
snapshotVxp = zeros(nzs, nxs, nt);
snapshotVzs = zeros(nzs, nxs, nt);
snapshotVxs = zeros(nzs, nxs, nt);
...
for it = 1:nt
    ...
    snapshotVxp(:, :, it) = Vxp(izs, ixs, 2);
    snapshotVzp(:, :, it) = Vzp(izs, ixs, 2);
    snapshotVxs(:, :, it) = Vxs(izs, ixs, 2);
    snapshotVzs(:, :, it) = Vzs(izs, ixs, 2);
