...
if (isunix) % Linux / MacOS
    ...
    mex CC='/usr/local/bin/mpicc' LD='/usr/local/bin/mpicc' CFLAGS='\$CFLAGS -std=c99' mpi_init.c
    mex CC='/usr/local/bin/mpicc' LD='/usr/local/bin/mpicc' CFLAGS='\$CFLAGS -std=c99' mpi_finalize.c
    ...
    mex CC='/usr/local/bin/mpicc' LD='/usr/local/bin/mpicc' CFLAGS='\$CFLAGS -std=c99' rvsTimeCpmlFor2dAw_openmpi_mex.c finiteDifference.c
end
