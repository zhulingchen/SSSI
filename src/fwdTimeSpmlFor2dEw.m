...
for it=1:nt
    ...
    A(iz,ix,2) = A_para(iz,ix,2) + A_perp(iz,ix,2);
    
    B(iz,ix,2) = B_para(iz,ix,2) + B_perp(iz,ix,2);
    
    Vzp(iz,ix,3) = (1-0.5.*zDamp(iz,ix).*dt) ./ (1+0.5.*zDamp(iz,ix).*dt) .* Vzp(iz,ix,2) ...
        + 0.25.*(vp2(iz+1,ix)+vp2(iz+1,ix+1)+vp2(iz,ix)+vp2(iz,ix+1)) .* TOZ ./ (1+0.5.*zDamp(iz,ix).*dt) .* ...
    ...
    Vxp(iz,ix,3) = (1-0.5.*xDamp(iz,ix).*dt)./(1+0.5.*xDamp(iz,ix).*dt).*Vxp(iz,ix,2)...
        + vp2(iz,ix) .*TOX ./ (1+0.5.*xDamp(iz,ix).*dt) .* ...
    ...
    Vzs(iz,ix,3) = (1-0.5.*xDamp(iz,ix).*dt) ./ (1+0.5.*xDamp(iz,ix).*dt) .* Vzs(iz,ix,2) ...
        - 0.25.*(vs2(iz,ix)+vs2(iz,ix-1)+vs2(iz-1,ix)+vs2(iz-1,ix-1)) .* TOX ./ (1+0.5.*xDamp(iz,ix).*dt) .* ...
    ...
    Vxs(iz,ix,3) = (1-0.5.*zDamp(iz,ix).*dt)./(1+0.5.*zDamp(iz,ix).*dt).*Vxs(iz,ix,2)...
        + vs2(iz,ix) .* TOZ ./(1+0.5.*zDamp(iz,ix).*dt) .* ...
