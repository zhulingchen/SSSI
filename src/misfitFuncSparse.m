function [value, grad] = misfitFuncSparse(dcoeff, transformFunc, w, fs, nShots, dataDeltaFreq, greenFreqForShotSet, greenFreqForRecSet)
% MISFITFUNCSPARSE Calculate the least-squares misfit function with respect
% to the coefficients of perturbation model dm under sparse transform
%
% value = 1/2 * (L(transform(dcoeff)) - delta_d)' * (L(transform(dcoeff)) - delta_d)
% grad = invTransform(L'(L(transform(dcoeff)) = delta_d))
%
%
...
grad = real(transformFunc(grad, 2));
